    ...
    function lh(){
        ...
        this.addTab = function(tabs, url, name, chat_id, focusTab, position) {
            ...
            $.get(url, function(data) {
                ...
                const messageContainerEle = dummy.firstChild
                const messageEleList = messageContainerEle.querySelectorAll('.response > .msg-body')

                messageEleList.forEach((ele) => {
                    if(ele.parentNode.classList.contains('response')){
                        let translate_lang = window.sessionStorage.getItem("translate_lang")
                        if (translate_lang===null) translate_lang = "en"
                        let xhr = new XMLHttpRequest();
                        xhr.open("POST", `${process.env.TRANSLATE_SERVICE_URL}/api/translate`, false);
                        xhr.setRequestHeader("Content-Type", "application/json");
                        xhr.send(JSON.stringify({text:ele.innerText, target_lang:translate_lang})); 
                        if (xhr.status === 200) {
                            ele.innerText = JSON.parse(xhr.response)
                        } else {
                ...
                $('#chat-id-'+chat_id).html(dummy.innerHTML);  
                // $('#chat-id-'+chat_id).html(data);  
                $('#CSChatMessage-'+chat_id).focus();
                    ...
                    if (data.error == 'false')
                    {
                        if (data.result != 'false')
                        {
                            ...
                            $.each(data.result,function(i,item) {
                                ...
                                // Translation start
                                const dummy = document.createElement('div')
                                dummy.innerHTML = item.content
                                const messageContainerEle = dummy.firstChild
                                const messageEle = messageContainerEle.querySelector('.msg-body')

                                if (messageContainerEle.classList.contains('message-admin') && window.sessionStorage.getItem("translate_lang") === null) {
                                    let xhr = new XMLHttpRequest();
                                    xhr.open("POST", `${process.env.TRANSLATE_SERVICE_URL}/api/getLang`, false);
                                    xhr.setRequestHeader("Content-Type", "application/json");
                                    xhr.send(JSON.stringify({text:messageEle.innerText})); 
                                    if (xhr.status === 200) {
                                        window.sessionStorage.setItem("translate_lang", JSON.parse(xhr.response))
                                    } else {
                                        console.error(xhr.statusText);
                                    }
                                }
                                ...
                                if(messageContainerEle.classList.contains('response')){
									const xhr = new XMLHttpRequest();
									const translate_lang = window.sessionStorage.getItem("translate_lang")
                                    xhr.open("POST", `${process.env.TRANSLATE_SERVICE_URL}/api/translate`, false);
                                    xhr.setRequestHeader("Content-Type", "application/json");
                                    xhr.send(JSON.stringify({text:messageEle.innerText, target_lang:translate_lang})); 
                                    if (xhr.status === 200) {
                                        messageEle.innerText = JSON.parse(xhr.response)
                                        item.content = messageContainerEle.outerHTML
                                    } else {
                                        console.error(xhr.statusText);
                                    }
                                }
