...
const initialState = fromJS({
    ...
    chatLiveData : {'lmsop':0, 'vtm':0,'otm':0, 'msop':0, 'uid' : 0, 'error' : '','lmsgid' : 0, 'operator' : '', 'messages' : [], 'closed' : false, 'ott' : '', 'status_sub' : 0, 'status' : 0}, // Stores live chat data
    chatStatusData : {},
    ...
    switch (action.type) {
        ...
        case ENDED_CHAT : {
            return state.set('shown',false)
                ...
                .set('chatLiveData',fromJS({'lmsop':0, 'vtm':0, 'otm':0, 'msop':0, 'uid':0, 'status' : 0, 'status_sub' : 0, 'uw' : false, 'ott' : '', 'closed' : false, 'lmsgid' : 0, 'operator' : '', 'messages' : []}))
        ...
        // Translation start
                ...
                dummy.innerHTML = action.data.messages
                const messageContainerEle = dummy.firstChild
                const messageEle = messageContainerEle.querySelector('.msg-body')

                if (messageContainerEle.classList.contains('response') && window.sessionStorage.getItem("translate_lang") === null) {
                ...
                if(messageContainerEle.classList.contains('message-admin')){
            ...
            xhr.setRequestHeader("Content-Type", "application/json");
                    ...
                    if (xhr.status === 200) {
                        ...
                        action.data.messages = messageContainerEle.outerHTML
                    } else {
                        console.error(xhr.statusText);
                    }
                }
